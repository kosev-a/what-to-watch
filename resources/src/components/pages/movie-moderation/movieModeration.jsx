...
export default MovieModeration;

function MovieModeration(props) {
    const apiUrl = import.meta.env.VITE_APP_URL;

    // Получение данных о фильмах
    const { data, isLoading, error } = useQuery({
        queryKey: ["filmsModeration"], // Уникальный ключ
        queryFn: async () => {
            const response = await axios.get(`${apiUrl}/api/films/moderation`);
                    ...
                    {data &&
                        Object.keys(data).map((key) => {
                            return (
                                <div
                                    key={key}
                                    className="movie-moderation__film-info"
                                >
                                    ...
                                    <Link
                                        to={`/edit-movie/${Object.values(data[key])[0]}`}
